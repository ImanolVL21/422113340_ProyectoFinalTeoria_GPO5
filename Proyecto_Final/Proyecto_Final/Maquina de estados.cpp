...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_M])//comenzar a caminar
	{
		dogAnim = 1;
...
void Animation() {
	...
	if (dogAnim == 1) { //animacion de caminar (estado 1)
		if (!step) {    // estado 1
			RLegs += 0.03f;
			FLegs += 0.03f; 
			head += 0.03f; 
			tail += 0.03f;
			if (RLegs > 15.0f) {//condicion para detenerlo, solo realiza un paso
				step = true;
			}
		}
		else { //estremidades giren en sentido contrario
			RLegs -= 0.03f;
			FLegs -= 0.03f;
			head -= 0.03f;
			tail -= 0.03f;
